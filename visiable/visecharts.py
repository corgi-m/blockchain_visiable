...
def setnodes(nodes, from_or_to):
    ...
    for layer in nodes:
        ...
        for node in layer:
            ...
            print(fillcolor)
            enodes.append(GraphNode(name=node.address, symbol_size=(5 - i) * 30),)
        return enodes

    def setedges(edges, from_or_to):
        eedges = []
        for edge in edges:
            eedges.append({"source": edge.nodefrom.address, "target": edge.nodeto.address})
        return eedges

    def drawecharts(nodes, edges):
        c = (
            Graph(init_opts=InitOpts(renderer='svg', width='8000px', height='4000px'))
                .add("", nodes, edges, repulsion=8000, layout='force')
